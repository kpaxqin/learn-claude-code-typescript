...
function extractClasses(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (m) {
      const name = m[1];
      const startLine = i + 1;
      // Find end of class: next top-level class/function declaration, or EOF
      let endLine = lines.length;
      for (let j = i + 1; j < lines.length; j++) {
        if (
          lines[j].match(/^(?:export\s+)?class\s/) ||
          lines[j].match(/^(?:export\s+)?(?:async\s+)?function\s/) ||
          lines[j].match(/^(?:export\s+)?const\s+\w+\s*=\s*(?:async\s+)?\(/)
        ) {
          endLine = j;
          break;
        }
...
function extractFunctions(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (m) {
      const isAsync = lines[i].includes("async function");
      functions.push({
        name: m[1],
        signature: `${isAsync ? "async " : ""}function ${m[1]}(${m[2]})`,
        startLine: i + 1,
      });
    }
  }
